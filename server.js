...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
        ...
        // Allow any participant to control, load media, and sync timeline instantly (lowest ms latency)
        if (msg.type === 'PLAY_YOUTUBE' || msg.type === 'LOAD_VIDEO' || msg.type === 'MEDIA_SYNC' || msg.type === 'SYNC_TIME' || msg.type === 'CONTROL') {
            if (!boundRoom || !rooms[boundRoom]) return;
            if (msg.type === 'PLAY_YOUTUBE' || msg.type === 'LOAD_VIDEO') {
