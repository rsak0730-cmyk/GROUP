...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (raw) => {
        ...
        if (msg.type === 'SEARCH_VIDEOS') {
            if (!msg.query) return;
            try {
                console.log(`Searching YouTube for: "${msg.query}"`);
                const searchResult = await ytSearch(msg.query);
                const videos = (searchResult && searchResult.videos ? searchResult.videos : []).slice(0, 6).map(v => ({
                    title: v.title,
                    videoId: v.videoId,
